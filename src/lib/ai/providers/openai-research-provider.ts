...
const RESEARCH_PROMPTS: Record<ExtendedResearchType, (q: ResearchRequest['query']) => string> = {
  ...
  channels: (q) =>
    ...
    (q.description ? ` Контекст компании: ${q.description}.` : '') +
    ` ЧАСТЬ A — какие каналы клиент УЖЕ использует: ищи конкретные признаки (объявления на Авито, кампании Яндекс.Директ, сообщества/реклама ВК, Telegram-канал, профиль 2ГИС/Яндекс.Карты, рассылки). Указывай канал как используемый ТОЛЬКО при наличии признака с URL. Если признаков не нашёл — НЕ пиши «не использует», а отметь «по этому каналу данных не найдено».` +
