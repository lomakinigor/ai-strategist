...
const RESEARCH_PROMPTS: Record<ExtendedResearchType, (q: ResearchRequest['query']) => string> = {
  ...
  competitors: (q) => {
    ...
    return (
      ...
      ` 3) ВСЕГДА добавляй одного конкурента, которого ты считаешь наиболее релевантным, пометь «[выбор AI]».` +
      ` Для каждого конкурента: название, URL, позиция на рынке, ценовой сегмент, сильные/слабые стороны.` +
