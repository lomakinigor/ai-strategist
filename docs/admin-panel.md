...
- middleware на `/admin/*` (включая `/archive`, `/admin/costs`, `/admin/usage`, `/admin/leads`)
- Заменить `ADMIN_ARCHIVE_PASSWORD` на полноценный логин

### 2.2 — Лиды (CRM-lite) ✅ DONE

Реализовано в этой сессии. Доступно: `/admin/leads`.

Что работает:

- Schema расширена: `leads.status` (enum `new/in_progress/closed`), `leads.utm` (jsonb), `leads.admin_notes`, `leads.updated_at`
- API `/api/lead` теперь сохраняет UTM в БД (раньше только пересылал в TG)
- Server actions: `updateLeadStatus`, `updateLeadNotes` (revalidatePath)
- 5 KPI: всего / новые / в работе / закрытые / paid vs retainer
- Фильтры через URL: `?status=new|in_progress|closed|all` + `?type=paid|retainer|all`
- Поиск: GET-form `?q=` по email/имени/компании (ilike)
- Inline-смена статуса (`<select>` → server action, цветовые бейджи)
- Inline-заметки (toggle textarea, кнопка «Сохранить», feedback ✓/ошибка)
- Раскрывающаяся строка с сообщением клиента
- Mailto-ссылки на email
- Лимит 200 строк, статистика по всей таблице
- Ссылка в admin-навигации

Что НЕ реализовано (можно добавить позже):

- Экспорт CSV / XLSX выборки
- Bulk-action (изменить статус для нескольких лидов сразу)
- Заметки с историей изменений (сейчас просто перезаписываются)
- Связка лид → research_job (если клиент пошёл по бесплатному после заявки)
...
### 2.5 — Доработка лендинга и причёсывание кнопок ✅ DONE (paid-first pivot)

Реализовано 2026-06-19. Главное содержательное изменение: **разворот лендинга с free-first на paid-first + speed claim 10 минут вместо 24 часов**.

Что сделано:

- `app/page.tsx` — главный лендинг переписан под paid-first
- `app/crisis/page.tsx` — кризисный лендинг переписан под paid-first
- `app/offer/page.tsx` — оферта обновлена (10 минут SLA)
- `app/lead/[type]/page.tsx` — описание тарифа обновлено
- Memory `project_landing_paid_first_2026_06.md` создан с полным описанием разворота
- MEMORY.md обновлён

Ключевые изменения контента:

- **H1** обоих лендингов содержит цену 9 999 ₽ и speed claim
- **Hero primary CTA** ведёт на `/intake?tier=paid` (цель Я.Метрики `paywall_click`)
- **Hero secondary** — маленькая текст-ссылка «или попробовать бесплатный пробник»
- **TARIFFS** — Paid featured с badge «Главный продукт», Free переименован в «Пробник» и сжат
- **STEPS** на главной — теперь 3 шага вокруг paid-flow: анкета → оплата СБП → отчёт на email
- **COMPARISON** — speed: «10 минут» вместо «24 часа»
- **Финальный CTA** обоих лендингов — paid-first
- **Все упоминания «24 часа»** заменены на «10 минут» (включая metadata, dark-промо, sub-тексты)

Дизайн-система НЕ менялась — те же `lp-*` классы, Inter, navy, amber.
...
- Канонический список UTM-меток для каждого канала (см. memory: каналы РФ — MAX, ВК, 2ГИС, Яндекс первичные; Telegram/WhatsApp/email вторичные)
- Шаблоны ссылок с метками: `/?utm_source=vk&utm_medium=community&utm_campaign=launch` и т.п.
- Возможно: маленький URL-builder в `/admin` для генерации/копирования ссылок
- Что отправляем по каждому каналу: тексты постов, заголовки, дизайн картинок (если нужны)
- Аналитика: воронка по utm_source в `/admin/usage` (требует доработки — backlog 1.2)
- Связка с метриками /admin/usage и Яндекс.Метрика goals (уже есть)
