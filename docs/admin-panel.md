...
### 2.5 — Доработка лендинга и причёсывание кнопок (next)

Запрошено пользователем 2026-06-19. Триггер: «после 2.2 → доработка лендинга и причесование кнопок».

Что нужно уточнить с пользователем перед стартом:

- Какой именно лендинг — главный `/` или `/crisis`, или оба?
- Что не нравится в кнопках (цвет / размер / hover / порядок / количество)?
- Нужен ли единый стиль кнопок через `lp-btn-primary/ghost` или редизайн?
- Прогнать ли `/ux-audit` перед началом, чтобы получить чек-лист улучшений?
