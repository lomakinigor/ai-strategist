...
| **Статус** | Done |
| **Feature** | F-007 |
| **Описание** | Генерация структурированного отчёта по 5 разделам через Anthropic API. Промпт формируется из верифицированных фактов RAG по всем 4 потокам. Явная маркировка [ФАКТ]/[ГИПОТЕЗА]/[НЕДОСТАТОЧНО ДАННЫХ]. |
...
**Отчёт:** Выполнено 2026-04-28. Создан `src/lib/strategy/generator.ts`: `generateStrategyDraft(jobId)` — строит RAG-контекст → формирует промпт → вызывает Anthropic API (claude-sonnet-4-6) или mock-заглушку → сохраняет в `report_artifacts`. Создан `src/lib/strategy/prompts.ts`: системный промпт с явными антигаллюцинационными правилами + `buildStrategyUserPrompt(context)`. Парсинг 5 разделов через `parseSections(markdown)`. Сохранение: `report_artifacts.status` = generating → done/error. Trigger: Server Action `generateStrategyAction` → redirect на `/research/[id]/report`. UI: кнопка «Сгенерировать стратегию» в research page при status=done; если отчёт уже есть — кнопка «Смотреть стратегию». Minimal report viewer: `/research/[id]/report` — 5 цветных секций. 97/97 тестов ✓, `npm run build` ✓.
...
| **Статус** | Done |
| **Feature** | F-008, F-009 |
| **Описание** | UI отчёта с 5 разделами (бизнес / рынок / аудитория / каналы / стратегия). Маркировка утверждений. Рекомендации по автоматизации. Базовая инфраструктура экспорта. Формат MVP — рабочий и временный. |
| **Тест-критерий** | Все 5 разделов присутствуют. Все ключевые утверждения маркированы. Текст на русском, суммы в рублях. Экспорт .md работает без ошибок. |

**Отчёт:** Выполнено 2026-04-28. Создан `app/research/[id]/report/CopyButton.tsx` ('use client', копирование в буфер). Обновлён `app/research/[id]/report/page.tsx`: визуальная маркировка [ФАКТ]/[ГИПОТЕЗА]/[НЕДОСТАТОЧНО ДАННЫХ] построчным renderer'ом, дисклеймер о достоверности данных с легендой, кнопка «Скопировать стратегию» и ссылка «Скачать .md». Создан `app/api/export/[artifactId]/route.ts`: GET-endpoint, возвращает contentMarkdown как attachment с filename по имени компании. Создан `app/api/export/__tests__/route.test.ts`: 4 теста (not found, generating, done+headers, default slug). 101/101 тестов ✓, `npm run build` ✓.
