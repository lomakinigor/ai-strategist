...
export async function GET(_req: Request, { params }: { params: { jobId: string } }) {
  ...
  // Артефакт done. Для free ждём briefJson (на /free-report без него видна
  // плашка «в обработке»). Для paid сразу редирект на /brief.
  const tier = artifact.tier ?? job.tier

  if (tier === 'free') {
    if (!artifact.briefJson) {
      const body: StatusResponse = {
        stage: 'brief_generating',
        jobStatus: job.status,
        streams,
        redirectTo: null,
        errorMessage: null,
  ...
  // paid → /brief (neon-дашборд)
  const body: StatusResponse = {
    stage: 'done',
    jobStatus: job.status,
    streams,
    redirectTo: `/brief/${artifact.id}`,
    errorMessage: null,
  }
  return NextResponse.json(body)
}
