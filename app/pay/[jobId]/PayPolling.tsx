...
export function PayPolling({ jobId }: PayPollingProps) {
  ...
  if (status === 'paid') {
    return (
      <div className="lp-card p-6 mb-8 border-2 border-[#1e3a8a]/30 bg-[#1e3a8a]/10">
        <p className="text-sm font-semibold text-[#1e3a8a] mb-1">✓ Оплата подтверждена</p>
        <p className="text-sm text-[#1e3a8a]">Запускаю исследование… секунду…</p>
      </div>
    )
  }
  ...
  if (timedOut) {
    ...
    return (
      <div className="lp-card p-6 mb-8 border-2 border-[#1e3a8a]/30 bg-[#1e3a8a]/10">
        ...
        <p className="text-sm text-[#1e3a8a] leading-[1.55] mb-3">
        ...
        <div className="flex flex-wrap gap-3 text-sm">
          <Link
            href="/intake"
            className="text-[#1e3a8a] font-medium underline hover:text-[#1e3a8a]"
          >
            ← Вернуться к анкете
          </Link>
  ...
  return (
    <div className="lp-card p-6 mb-8 border-2 border-[#1e3a8a] bg-[#fafafa]">
