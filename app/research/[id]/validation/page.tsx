...
const STREAM_COLORS: Record<ResearchType, string> = {
  business: 'bg-[#1e3a8a]/10 text-[#1e3a8a]',
  market: 'bg-[#1e3a8a]/10 text-[#1e3a8a]',
  audience: 'bg-[#1e3a8a]/10 text-[#1e3a8a]',
  channels: 'bg-[#1e3a8a]/10 text-[#1e3a8a]',
  competitors: 'bg-[#1e3a8a]/10 text-[#1e3a8a]',
}
...
const FACT_TYPE_COLORS: Record<FactType, string> = {
  FACT: 'bg-[#1e3a8a]/10 text-[#1e3a8a]',
  HYPOTHESIS: 'bg-[#fafafa] text-[#0a0a0a] border border-[#1e3a8a]/30',
  INSUFFICIENT_DATA: 'bg-white text-[#525252] border border-dashed border-black/10',
}
...
const CONFIDENCE_COLORS: Record<ConfidenceLevel, string> = {
  HIGH: 'bg-[#1e3a8a]/10 text-[#1e3a8a]',
  MEDIUM: 'bg-[#fafafa] text-[#0a0a0a]',
  LOW: 'bg-white text-[#525252] border border-dashed border-black/10',
}
...
export default async function ValidationPage({
  ...
  const modeBadgeClass =
    researchMode === 'real' ? 'text-[#1e3a8a] bg-[#1e3a8a]/10' : 'text-[#525252] bg-[#fafafa]'
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="mb-6">
          <div className="flex items-start justify-between">
            <div>
              <a href={`/research/${params.id}`} className="text-xs text-[#1e3a8a] hover:underline mb-1 block">
                ← Назад к статусу исследования
              </a>
              <h1 className="text-2xl font-bold text-[#525252]">{company?.name ?? 'Компания'}</h1>
              {company?.industry && (
                <p className="text-sm text-[#525252] mt-0.5">{company.industry}</p>
              )}
        ...
        {allFacts.length === 0 && (
          <div className="bg-white border border-black/10 rounded-lg p-8 text-center">
            ...
            <a
              ...
              className="mt-3 inline-block text-sm text-[#1e3a8a] hover:underline"
            >
        ...
        {allFacts.length > 0 && (
          <>
            ...
            <div className="bg-white border border-black/10 rounded-lg p-4 mb-4">
              {/* Stream filter */}
              <div className="mb-3">
                <p className="text-xs font-medium text-[#525252] mb-2">Поток</p>
                    ...
                    return (
                      <a
                        ...
                        className={`px-3 py-1 rounded-full text-xs font-medium border transition-colors ${
                          selected
                            ? 'bg-[#1e3a8a] text-white border-[#1e3a8a]/30'
                            : 'bg-white text-[#525252] border-black/10 hover:border-[#1e3a8a]/30'
                        }`}
                      >
              ...
              {/* Fact type filter */}
              <div className="mb-3">
                <p className="text-xs font-medium text-[#525252] mb-2">Тип факта</p>
                <div className="flex flex-wrap gap-2">
              ...
              {/* Confidence filter */}
              <div className="mb-3">
                <p className="text-xs font-medium text-[#525252] mb-2">Достоверность</p>
                <div className="flex flex-wrap gap-2">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-black/10">
                ...
                {hasFilters && (
                  <a
                    ...
                    className="text-xs text-[#525252] hover:text-[#1e3a8a] hover:underline"
                  >
                    Сбросить фильтры
                  </a>
                )}
            ...
            {filteredFacts.length === 0 && hasFilters && (
              <div className="bg-white border border-black/10 rounded-lg p-6 text-center">
                ...
                <a
                  href={`/research/${params.id}/validation`}
                  className="mt-2 inline-block text-sm text-[#1e3a8a] hover:underline"
                >
                ...
                {filteredFacts.map((fact) => (
                  <div
                    ...
                    className={`bg-white border rounded-lg p-4 transition-opacity ${
                      fact.isActive ? 'border-black/10' : 'border-black/10 opacity-60'
                    }`}
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-2">
                          ...
                          <span className="inline-block text-xs font-sans tabular-nums px-2 py-0.5 rounded-full bg-[#fafafa] text-[#525252]">
                            RS:{fact.rs}
                          </span>
                        </div>

                        {/* Text */}
                        <p className="text-sm text-[#525252] leading-relaxed">{fact.content}</p>
                          ...
                          <div className="mt-2 flex items-center gap-2">
                            {fact.source.url ? (
                              <a
                                ...
                                className="text-xs text-[#1e3a8a] hover:underline truncate max-w-xs"
                                title={fact.source.url}
                              >
                                {new URL(fact.source.url).hostname}
                              </a>
                            ) : (
                              <span className="text-xs text-[#525252]">{fact.source.label}</span>
                            )}
                            <span className="text-xs text-[#525252] bg-[#fafafa] border border-black/10 px-1.5 py-0.5 rounded">
            ...
            <div className="mt-6 pt-4 border-t border-black/10 flex items-center justify-between">
              <p className="text-xs text-[#525252]">
                Активных фактов для отчёта: <span className="font-medium text-[#525252]">{totalActive}</span>
              </p>
              {totalActive === 0 ? (
                <button
                  type="button"
                  disabled
                  className="px-4 py-2 rounded-lg text-sm font-medium bg-[#fafafa] text-[#525252] cursor-not-allowed"
                >
                ...
                <Link
                  ...
                  className="px-4 py-2 rounded-lg text-sm font-medium bg-[#1e3a8a] text-white hover:bg-[#1e3a8a] transition-colors"
                >
